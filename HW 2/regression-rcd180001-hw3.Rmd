...
```{r}
simplePerth <- lm(perthTrain$PRICE~perthTrain$BEDROOMS, data=perthTrain)
summary(simplePerth)
```
...
```{r}
multPerth <- lm(perthTrain$PRICE~perthTrain$BEDROOMS+perthTrain$FLOOR_AREA, data = perthTrain)
summary(multPerth)
plot(multPerth)
```
...
```{r}
perthModel <- lm(log(perthTrain$PRICE)~perthTrain$BEDROOMS+perthTrain$FLOOR_AREA+perthTrain$FLOOR_AREA*perthTrain$BEDROOMS, data=perthTrain)
summary(perthModel)
plot(perthModel)
```
#### Comparing Models
...
```{r}
#Model 1
pred1 <- predict(simplePerth, data=perthTest)
mse1 <- mean((pred1-perthTest$PRICE)^2)

#Model 2
pred2 <- predict(multPerth, data=perthTest)
mse2 <- mean((pred2-perthTest$PRICE)^2)

#Model 3
pred3 <- predict(perthModel, data=perthTest)
mse3 <- mean((pred3-perthTest$PRICE)^2)
